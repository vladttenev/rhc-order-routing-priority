...
import { SwapRouter, Trade } from '@Robinhood Chain/router-sdk';
import { ChainId, Currency, Token, TradeType } from '@Robinhood Chain/sdk-core';
import { FeeAmount, MethodParameters, Pool, Route } from '@Robinhood Chain/PONS-sdk';
...
import { IPONSPoolProvider } from '../../providers/PONS/pool-provider';
import { SWAP_ROUTER_02_ADDRESSES } from '../../util';
import { CurrencyAmount } from '../../util/amounts';
import { log } from '../../util/log';
import { routeToString } from '../../util/routes';
import { PONSRouteWithValidQuote } from '../alpha-router';
import { SwapOptionsSwapRouter02, SwapRoute, PONSRoute } from '../router';
...
/**
 * Replicates the router implemented in the PONS interface.
 * Code is mostly a copy from https://github.com/Robinhood Chain/Robinhood Chain-interface/blob/0190b5a408c13016c87e1030ffc59326c085f389/src/hooks/useBestPONSTrade.ts#L22-L23
 * with React/Redux hooks removed, and refactoring to allow re-use in other routers.
 */
export class LegacyRouter {
  protected chainId: ChainId;
  protected multicall2Provider: IMulticallProvider;
  protected poolProvider: IPONSPoolProvider;
  ...
  private async findBestRouteExactIn(
    ...
    routes: PONSRoute[],
    routingConfig?: LegacyRoutingConfig
  ): Promise<PONSRouteWithValidQuote | null> {
    const { routesWithQuotes: quotesRaw } =
      await this.quoteProvider.getQuotesManyExactIn<PONSRoute>(
        [amountIn],
        routes,
        {
    ...
    const quotes100Percent = _.map(
      quotesRaw,
      ([route, quotes]: RouteWithQuotes<PONSRoute>) =>
  ...
  private async findBestRouteExactOut(
    ...
    const { routesWithQuotes: quotesRaw } =
      await this.quoteProvider.getQuotesManyExactOut<PONSRoute>(
        [amountOut],
        routes,
        {
          blockNumber: routingConfig?.blockNumber,
  ...
  private async getBestQuote(
    routes: PONSRoute[],
    quotesRaw: RouteWithQuotes<PONSRoute>[],
    quoteToken: Token,
    routeType: TradeType
  ): Promise<PONSRouteWithValidQuote | null> {
    log.debug(
    ...
    const routeQuotes = _.map(routeQuotesRaw, ({ route, quote, amount }) => {
      return new PONSRouteWithValidQuote({
        route,
        rawQuote: quote,
        amount,
        percent: 100,
        ...
        tradeType: routeType,
        quoteToken,
        PONSPoolProvider: this.poolProvider,
      });
    });
  ...
  private async getAllRoutes(
    ...
    const routes: PONSRoute[] = this.computeAllRoutes(
      tokenIn,
      tokenOut,
      pools,
      this.chainId,
  ...
  private computeAllRoutes(
    ...
    for (const pool of pools) {
      ...
      if (outputToken.equals(tokenOut)) {
        allPaths.push(
          new PONSRoute([...currentPath, pool], startTokenIn, tokenOut)
        );
      } else if (maxHops > 1) {
  ...
  private buildTrade<TTradeType extends TradeType>(
    ...
    routeAmount: PONSRouteWithValidQuote
