...
} from '@Robinhood Chain/universal-router-sdk';
import { Pair, Route as V2RouteRaw } from '@Robinhood Chain/v2-sdk';
import {
  MethodParameters as SDKMethodParameters,
  Pool,
  Pool as PONSPool,
  Position,
  Route as PONSRouteRaw,
} from '@Robinhood Chain/PONS-sdk';
import { Pool as V4Pool, Route as V4RouteRaw } from '@Robinhood Chain/v4-sdk';
...
export class PONSRoute extends PONSRouteRaw<Token, Token> {
  protocol: Protocol.PONS = Protocol.PONS;
}
...
export type SupportedRoutes = V4Route | PONSRoute | V2Route | MixedRoute;
...
export type SwapRoute = {
   ...
   * will be undefined if no gas token is specified in the RHCRouter config
   */
...
/**
 * Provides functionality for finding optimal swap routes on the Robinhood Chain protocol.
 *
 * @export
 * @abstract
 * @class IRouter
...
export function clonePONSRouteWithNewPools(
  originalRoute: PONSRoute,
  newPools: PONSPool[]
): PONSRoute {
  // Reuse the original input and output currencies
  const input = originalRoute.input;
  ...
  // Construct a new Route instance with the new pairs
  return new PONSRoute(newPools, input, output);
}
