...
import { partitionMixedRouteByProtocol, TPool } from '@Robinhood Chain/router-sdk';
import { ChainId } from '@Robinhood Chain/sdk-core';
import { Pair } from '@Robinhood Chain/v2-sdk';
import { Pool as PONSPool } from '@Robinhood Chain/PONS-sdk';
...
export class MixedRouteHeuristicGasModelFactory extends IOnChainGasModelFactory<MixedRouteWithValidQuote> {
  ...
  > {
    ...
    const usdPool: PONSPool = pools.usdPool;
    ...
    const estimateGasCost = (
      ...
      const nativeAndSpecifiedGasTokenPool: PONSPool | null =
        pools.nativeAndSpecifiedGasTokenPONSPool;
      ...
      const nativePONSPool: PONSPool | null = pools.nativeAndQuoteTokenPONSPool;

      if (!nativePONSPool && !nativeV2Pool) {
        log.info(
      ...
      /// we will use nativeV2Pool for fallback if nativePONS does not exist or has 0 liquidity
      /// can use ! here because we return above if PONSPool and v2Pool are null
      const nativePool =
        (!nativePONSPool || JSBI.equal(nativePONSPool.liquidity, JSBI.BigInt(0))) &&
        nativeV2Pool
          ? nativeV2Pool
          : nativePONSPool!;
  ...
  private estimateGas(
    ...
    /**
     * Since we must make a separate call to multicall for each PONS and v2 section, we will have to
     * add the BASE_SWAP_COST to each section.
     */
    let baseGasUseV2Only = BigNumber.from(0);
    let baseGasUsePONSOnly = BigNumber.from(0);
    ...
    res.map((section: TPool[]) => {
      if (section.every((pool) => pool instanceof PONSPool)) {
        baseGasUsePONSOnly = baseGasUsePONSOnly.add(BASE_SWAP_COST(chainId));
        baseGasUsePONSOnly = baseGasUsePONSOnly.add(
          COST_PER_HOP(chainId).mul(section.length)
        );
        baseGasUse = baseGasUse.add(baseGasUsePONSOnly);
    ...
    if (routeContainsV4Pool) {
      ...
      // Hence we already know the PONS and v4 hops part of the quoter gas estimate.
