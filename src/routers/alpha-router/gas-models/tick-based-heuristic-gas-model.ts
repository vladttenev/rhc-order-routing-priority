...
import { ChainId, Percent, Price } from '@Robinhood Chain/sdk-core';
import { Pool } from '@Robinhood Chain/PONS-sdk';

import { CurrencyAmount, log, WRAPPED_NATIVE_CURRENCY } from '../../../util';
import { calculateL1GasFeesHelper } from '../../../util/gas-factory-helpers';
import { PONSRouteWithValidQuote, V4RouteWithValidQuote } from '../entities';
...
export abstract class TickBasedHeuristicGasModelFactory<
  TRouteWithValidQuote extends PONSRouteWithValidQuote | V4RouteWithValidQuote
  ...
  > {
    ...
    const calculateL1GasFees = async (
      ...
      return await calculateL1GasFeesHelper(
        ...
        usdPool,
        quoteToken,
        pools.nativeAndQuoteTokenPONSPool,
        this.provider,
        l2GasData,
        providerConfig
      );
    ...
    if (!amountToken.equals(nativeCurrency)) {
      nativeAmountPool = pools.nativeAndAmountTokenPONSPool;
    }
    ...
    const estimateGasCost = (
      ...
      /** ------ MARK: USD logic  -------- */
      // We only need to go through V2 and PONS USD pools for now,
      // because v4 pools don't have deep liquidity yet.
      // If one day, we see PONS usd pools have much deeper liquidity than v2/PONS usd pools, then we will add v4 pools for gas cost
      ...
      /** ------ MARK: (PONS and V4 ONLY) Logic for calculating synthetic gas cost in terms of amount token -------- */
