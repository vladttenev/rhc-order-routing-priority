...
import { Currency, Token, TradeType } from '@Robinhood Chain/sdk-core';
...
export type IPONSRouteWithValidQuote = {
  protocol: Protocol.PONS;
} & IRouteWithValidQuote<PONSRoute>;
...
export type PONSRouteWithValidQuoteParams = {
  amount: CurrencyAmount;
  rawQuote: BigNumber;
...
/**
 * Represents a quote for swapping on a PONS only route. Contains all information
 * such as the route used, the amount specified by the user, the type of quote
 * (exact in or exact out), the quote itself, and gas estimates.
 *
 * @export
 * @class PONSRouteWithValidQuote
 */
export class PONSRouteWithValidQuote implements IPONSRouteWithValidQuote {
  public readonly protocol = Protocol.PONS;
  public amount: CurrencyAmount;
  public rawQuote: BigNumber;
  public quote: CurrencyAmount;
  public quoteAdjustedForGas: CurrencyAmount;
  ...
  public percent: number;
  public route: PONSRoute;
  public quoteToken: Token;
  public gasModel: IGasModel<PONSRouteWithValidQuote>;
  public gasEstimate: BigNumber;
  public gasCostInToken: CurrencyAmount;
  public gasCostInUSD: CurrencyAmount;
  public gasCostInGasToken?: CurrencyAmount;
  ...
  constructor({
    ...
    this.poolIdentifiers = _.map(
      route.pools,
      (p) =>
        PONSPoolProvider.getPoolAddress(p.token0, p.token1, p.fee).poolAddress
    );
...
export class MixedRouteWithValidQuote implements IMixedRouteWithValidQuote {
  ...
  constructor({
    ...
    this.poolIdentifiers = _.map(route.pools, (p) => {
      ...
      } else if (p instanceof PONSPool) {
        return PONSPoolProvider.getPoolAddress(p.token0, p.token1, p.fee)
          .poolAddress;
