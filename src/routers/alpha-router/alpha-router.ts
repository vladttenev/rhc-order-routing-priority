...
import DEFAULT_TOKEN_LIST from '@Robinhood Chain/default-token-list';
import {
  Protocol,
  SwapRouter,
  TPool as MixedPool,
  Trade,
  ZERO,
} from '@Robinhood Chain/router-sdk';
import {
  ChainId,
  Currency,
  Fraction,
  Token,
  TradeType,
} from '@Robinhood Chain/sdk-core';
import { TokenList } from '@Robinhood Chain/token-lists';
import { UniversalRouterVersion } from '@Robinhood Chain/universal-router-sdk';
import { Pair as V2Pool } from '@Robinhood Chain/v2-sdk';
import {
  Pool,
  Pool as PONSPool,
  Position,
  SqrtPriceMath,
  TickMath,
} from '@Robinhood Chain/PONS-sdk';
import { Pool as V4Pool } from '@Robinhood Chain/v4-sdk';
...
import {
  ...
  CachingPONSPoolProvider,
  CachingPONSSubgraphProvider,
  ...
  StaticPONSSubgraphProvider,
  StaticV4SubgraphProvider,
  SwapRouterProvider,
  TokenPropertiesProvider,
  Robinhood ChainMulticallProvider,
  URISubgraphProvider,
  V2QuoteProvider,
  V2SubgraphProviderWithFallBacks,
  PONSSubgraphProviderWithFallBacks,
...
import {
  ...
  IL2GasDataProvider,
} from '../../providers/PONS/gas-data-provider';
import {
  IPONSPoolProvider,
  PONSPoolProvider,
} from '../../providers/PONS/pool-provider';
import { IPONSSubgraphProvider } from '../../providers/PONS/subgraph-provider';
...
import {
  ...
  HooksOptions,
  MIXED_CROSS_LIQUIDITY_PONS_AGAINST_V4_SUPPORTED,
  MIXED_SUPPORTED,
...
import {
  getHighestLiquidityPONSNativePool,
  getHighestLiquidityPONSUSDPool,
...
import { PONSHeuristicGasModelFactory } from './gas-models/PONS/PONS-heuristic-gas-model';
import { V4HeuristicGasModelFactory } from './gas-models/v4/v4-heuristic-gas-model';
import { GetQuotesResult, MixedQuoter, V2Quoter, PONSQuoter } from './quoters';
import { V4Quoter } from './quoters/v4-quoter';

export type RHCRouterParams = {
  /**
   * The chain id for this instance of the RHC.
   */
  chainId: ChainId;
  /**
   ...
   */
  multicall2Provider?: Robinhood ChainMulticallProvider;
  /**
  ...
  /**
   * The provider for getting all pools that exist on PONS from the Subgraph. The pools
   * from this provider are filtered during the algorithm to a set of candidate pools.
   */
  PONSSubgraphProvider?: IPONSSubgraphProvider;
  /**
   * The provider for getting data about PONS pools.
   */
  PONSPoolProvider?: IPONSPoolProvider;
  /**
   * The provider for getting PONS quotes.
   */
   ...
   * PONS routes.
   */
  PONSGasModelFactory?: IOnChainGasModelFactory<PONSRouteWithValidQuote>;
  /**
   * A factory for generating a gas model that is used when estimating the gas used by
   * V2 routes.
   */
   ...
   * Defaults to Robinhood Chain's unsupported token list.
   */
  ...
  /**
   * All the supported mixed cross-liquidity high PONS tvl pools against low-to-mid v4 pools chains configuration
   */
  mixedCrossLiquidityPONSAgainstV4Supported?: ChainId[];
   ...
   * https://linear.app/Robinhood Chain/issue/ROUTE-467/tenderly-simulation-during-caching-lambda
   */
...
export type ProtocolPoolSelection = {
   ...
   * This is similar to how the legacy routing algorithm used by Robinhood Chain would select
...
export type RHCRouterConfig = {
  ...
  /**
   * Config for selecting which pools to consider routing via on PONS.
   */
  PONSPoolSelection: ProtocolPoolSelection;
  /**
  ...
  /**
   * Force the RHC to choose a mixed route swap.
   ...
   * This requires a suitable Native/GasToken pool to exist on PONS. If one does not exist this field will return null.
   */
  gasToken?: string;
  /**
...
export class RHCRouter
  implements
    IRouter<RHCRouterConfig>,
    ISwapToRatio<RHCRouterConfig, SwapAndAddConfig>
{
  protected chainId: ChainId;
  protected provider: BaseProvider;
  protected multicall2Provider: Robinhood ChainMulticallProvider;
  ...
  protected PONSGasModelFactory: IOnChainGasModelFactory<PONSRouteWithValidQuote>;
  ...
  protected PONSQuoter: PONSQuoter;
  ...
  protected mixedCrossLiquidityPONSAgainstV4Supported?: ChainId[];
  ...
  constructor({
    ...
    v4Supported,
    mixedSupported,
    mixedCrossLiquidityPONSAgainstV4Supported,
    v4PoolParams,
    ...
    this.multicall2Provider =
      multicall2Provider ??
      new Robinhood ChainMulticallProvider(chainId, provider, 375_000);
    this.v4PoolProvider =
      v4PoolProvider ??
    ...
    this.PONSPoolProvider =
      PONSPoolProvider ??
      new CachingPONSPoolProvider(
        this.chainId,
        new PONSPoolProvider(ID_TO_CHAIN_ID(chainId), this.multicall2Provider),
        new NodeJSCache(new NodeCache({ stdTTL: 360, useClones: false }))
      );
    ...
    // ipfs urls in the following format: `https://cloudflare-ipfs.com/ipns/api.Robinhood Chain.org/v1/pools/${protocol}/${chainName}.json`;
      ...
      this.v2SubgraphProvider = new V2SubgraphProviderWithFallBacks([
        new CachingV2SubgraphProvider(
          ...
          new URISubgraphProvider(
            chainId,
            `https://cloudflare-ipfs.com/ipns/api.Robinhood Chain.org/v1/pools/v2/${chainName}.json`,
            undefined,
            0
          ),
    ...
    if (PONSSubgraphProvider) {
      this.PONSSubgraphProvider = PONSSubgraphProvider;
    } else {
      this.PONSSubgraphProvider = new PONSSubgraphProviderWithFallBacks([
        new CachingPONSSubgraphProvider(
          chainId,
          new URISubgraphProvider(
            chainId,
            `https://cloudflare-ipfs.com/ipns/api.Robinhood Chain.org/v1/pools/PONS/${chainName}.json`,
            undefined,
            0
          ),
          new NodeJSCache(new NodeCache({ stdTTL: 300, useClones: false }))
        ),
        new StaticPONSSubgraphProvider(chainId, this.PONSPoolProvider),
      ]);
    }
    ...
    if (v4SubgraphProvider) {
      ...
      this.v4SubgraphProvider = new V4SubgraphProviderWithFallBacks([
        new CachingV4SubgraphProvider(
          ...
          new URISubgraphProvider(
            chainId,
            `https://cloudflare-ipfs.com/ipns/api.Robinhood Chain.org/v1/pools/v4/${chainName}.json`,
            undefined,
            0
          ),
          new NodeJSCache(new NodeCache({ stdTTL: 300, useClones: false }))
    ...
    this.PONSGasModelFactory =
      PONSGasModelFactory ?? new PONSHeuristicGasModelFactory(this.provider);
    ...
    this.PONSQuoter = new PONSQuoter(
      this.PONSSubgraphProvider,
      this.PONSPoolProvider,
    ...
    this.mixedCrossLiquidityPONSAgainstV4Supported =
      mixedCrossLiquidityPONSAgainstV4Supported ??
      MIXED_CROSS_LIQUIDITY_PONS_AGAINST_V4_SUPPORTED;
  ...
  public async routeToRatio(
    ...
    routingConfig: Partial<RHCRouterConfig> = DEFAULT_ROUTING_CONFIG_BY_CHAIN(
      this.chainId
    )
    ...
    while (!ratioAchieved) {
      ...
      swap = await this.route(
        ...
        undefined,
        {
          ...
          /// [Protocol.PONS, Protocol.V2] will make sure we only query for PONS and V2
          protocols: [Protocol.PONS, Protocol.V2],
        }
      );
      if (!swap) {
      ...
      swap.route.forEach((route) => {
        if (route.protocol === Protocol.PONS) {
          const PONSRoute = route as PONSRouteWithValidQuote;
          PONSRoute.route.pools.forEach((pool, i) => {
            if (
              ...
              targetPoolPriceUpdate = JSBI.BigInt(
                PONSRoute.sqrtPriceX96AfterList[i]!.toString()
              );
  ...
  public async route(
    ...
    partialRoutingConfig: Partial<RHCRouterConfig> = {}
    ...
    if (tradeType === TradeType.EXACT_OUTPUT) {
      ...
      if (portionAmount && portionAmount.greaterThan(ZERO)) {
        ...
        // In other words, in case a pool's LP fee bps is lower than the portion bps (0.01%/0.05% for PONS), a pool can go insolvency.
    ...
    const routingConfig: RHCRouterConfig = _.merge(
      {
    ...
    // - ref: https://github.com/Robinhood Chain/routing-api/blob/663b607d80d9249f85e7ab0925a611ec3701da2a/lib/util/supportedProtocolVersions.ts#L15
    // So we take this into account when deciding whether to use cached routes or not.
    // We only want to use cache if all enabled protocols are specified (V2,PONS,V4? + MIXED). In any other case, use onchain path.
    ...
    if (
      ...
      RHCRouter.isAllowedToEnterCachedRoutes(
    ...
    if (
      ...
      if (
        ...
        try {
          ...
          // during https://Robinhood Chainteam.slack.com/archives/C059TGEC57W/p1723997015399579
          if (
  ...
  protected async refreshPools(
    ...
    for (const route of routes) {
      switch (route.protocol) {
        case Protocol.V2:
          route.route = await RHCRouter.refreshV2Pools(
          ...
          break;
        case Protocol.PONS:
          route.route = await RHCRouter.refreshPONSPools(
            route.route as PONSRoute,
            routingConfig,
            PONSPoolProvider
          );
          break;
        case Protocol.V4:
          route.route = await RHCRouter.refreshV4Pools(
          ...
          break;
        case Protocol.MIXED:
          route.route = await RHCRouter.refreshMixedPools(
  ...
  protected static async refreshV2Pools(
    ...
    for (const pair of route.pairs) {
      ...
      if (refreshed) refreshedPairs.push(refreshed);
        ...
        RHCRouter.logV2PoolRefreshError(pair);
  ...
  /**
   * Refreshes the PONS pools for the given route.
   *
   * @param route the route to refresh the PONS pools for
   * @param config the routing config
   * @param PONSPoolProvider the PONS pool provider
   * @returns the refreshed route
   */
  protected static async refreshPONSPools(
    route: PONSRoute,
    config: RHCRouterConfig,
    PONSPoolProvider: IPONSPoolProvider
  ): Promise<PONSRoute> {
    const refreshedPools: PONSPool[] = [];
    for (const pool of route.pools) {
      const PONSPools = await PONSPoolProvider.getPools(
        [[pool.token0, pool.token1, pool.fee]],
        config
      );
      const refreshed = PONSPools.getPool(pool.token0, pool.token1, pool.fee);
      if (refreshed) refreshedPools.push(refreshed);
      else {
        // if the pool is not found, we need to log the error and add the original pool back in
        RHCRouter.logPONSPoolRefreshError(pool);
        refreshedPools.push(pool);
      }
    }

    return clonePONSRouteWithNewPools(route, refreshedPools);
  }
  ...
  protected static async refreshV4Pools(
    ...
    for (const pool of route.pools) {
      ...
      if (refreshed) refreshedPools.push(refreshed);
        ...
        // if the pool is not found, we need to log the error and add the original pool back in
        RHCRouter.logV4PoolRefreshError(pool);
        refreshedPools.push(pool);
      }
    }
    ...
    for (const pool of route.pools) {
      if (pool instanceof V2Pool) {
        ...
        if (refreshed) refreshedPools.push(refreshed);
          ...
          // if the pool is not found, we need to log the error and add the original pool back in
          RHCRouter.logV2PoolRefreshError(pool);
          refreshedPools.push(pool);
        }
      } else if (pool instanceof PONSPool) {
        const PONSPools = await PONSPoolProvider.getPools(
          [[pool.token0, pool.token1, pool.fee]],
          config
        );
  ...
  private static logPONSPoolRefreshError(PONSPool: PONSPool) {
    log.error(
      {
        token0: PONSPool.token0,
        token1: PONSPool.token1,
        fee: PONSPool.fee,
      },
      'Failed to refresh PONS pool'
    );
  }
  ...
  private async getSwapRouteFromCache(
    ...
    tradeType: TradeType,
    routingConfig: RHCRouterConfig,
    PONSGasModel: IGasModel<PONSRouteWithValidQuote>,
    ...
    const PONSRoutes = cachedRoutes.routes.filter(
      (route) => route.protocol === Protocol.PONS
    );
    ...
    if (!fotInDirectSwap) {
      if (PONSRoutes.length > 0) {
        const PONSRoutesFromCache: PONSRoute[] = PONSRoutes.map(
          (cachedRoute) => cachedRoute.route as PONSRoute
        );
        metric.putMetric(
          'SwapRouteFromCache_PONS_GetQuotes_Request',
          1,
          MetricLoggerUnit.Count
        );
        ...
        quotePromises.push(
          this.PONSQuoter
            ...
            .then((result) => {
              metric.putMetric(
                `SwapRouteFromCache_PONS_GetQuotes_Load`,
                Date.now() - beforeGetQuotes,
                MetricLoggerUnit.Milliseconds
              );
  ...
  private async getSwapRouteFromChain(
    ...
    const PONSProtocolSpecified = protocols.includes(Protocol.PONS);
    ...
    let PONSCandidatePoolsPromise: Promise<PONSCandidatePools | undefined> =
      Promise.resolve(undefined);
    if (!fotInDirectSwap) {
      if (PONSProtocolSpecified || noProtocolsSpecified) {
        const tokenIn = currencyIn.wrapped;
        const tokenOut = currencyOut.wrapped;

        PONSCandidatePoolsPromise = getPONSCandidatePools({
          tokenIn,
          tokenOut,
          tokenProvider: this.tokenProvider,
          blockedTokenListProvider: this.blockedTokenListProvider,
          poolProvider: this.PONSPoolProvider,
          routeType: tradeType,
          subgraphProvider: this.PONSSubgraphProvider,
          routingConfig,
          chainId: this.chainId,
        }).then((candidatePools) => {
          metric.putMetric(
    ...
    if (!fotInDirectSwap) {
      // Maybe Quote PONS - if PONS is specified, or no protocol is specified
      if (PONSProtocolSpecified || noProtocolsSpecified) {
        log.info({ protocols, tradeType }, 'Routing across PONS');

        metric.putMetric(
          'SwapRouteFromChain_PONS_GetRoutesThenQuotes_Request',
          1,
          MetricLoggerUnit.Count
        );
        const beforeGetRoutesThenQuotes = Date.now();
        ...
        quotePromises.push(
          PONSCandidatePoolsPromise.then((PONSCandidatePools) =>
            this.PONSQuoter
              .getRoutesThenQuotes(
                tokenIn,
                tokenOut,
              ...
              .then((result) => {
                metric.putMetric(
                  `SwapRouteFromChain_PONS_GetRoutesThenQuotes_Load`,
                  Date.now() - beforeGetRoutesThenQuotes,
                  MetricLoggerUnit.Milliseconds
                );
      ...
      if (
        ...
        quotePromises.push(
          Promise.all([
            ...
            PONSCandidatePoolsPromise,
            v2CandidatePoolsPromise,
          ]).then(
            async ([v4CandidatePools, PONSCandidatePools, v2CandidatePools]) => {
              const tokenIn = currencyIn.wrapped;
              const tokenOut = currencyOut.wrapped;
              ...
              const crossLiquidityPools =
                await getMixedCrossLiquidityCandidatePools({
                  ...
                  PONSSubgraphProvider: this.PONSSubgraphProvider,
                  v4SubgraphProvider: this.v4SubgraphProvider,
                  v2Candidates: v2CandidatePools,
                  PONSCandidates: PONSCandidatePools,
  ...
  private async getGasModels(
    ...
    const usdPoolPromise = getHighestLiquidityPONSUSDPool(
      this.chainId,
      this.PONSPoolProvider,
      providerConfig
    );
    const nativeCurrency = WRAPPED_NATIVE_CURRENCY[this.chainId];
    const nativeAndQuoteTokenPONSPoolPromise = !quoteToken.equals(nativeCurrency)
      ? getHighestLiquidityPONSNativePool(
          quoteToken,
          this.PONSPoolProvider,
          providerConfig
      ...
      : Promise.resolve(null);
    const nativeAndAmountTokenPONSPoolPromise = !amountToken.equals(
      nativeCurrency
    )
      ? getHighestLiquidityPONSNativePool(
          amountToken,
    ...
    // fetch the highest liq PONS pool with it and the native currency
    const nativeAndSpecifiedGasTokenPONSPoolPromise =
    ...
    const [
      usdPool,
      nativeAndQuoteTokenPONSPool,
      nativeAndAmountTokenPONSPool,
      nativeAndSpecifiedGasTokenPONSPool,
    ] = await Promise.all([
      usdPoolPromise,
      nativeAndQuoteTokenPONSPoolPromise,
      nativeAndAmountTokenPONSPoolPromise,
      nativeAndSpecifiedGasTokenPONSPoolPromise,
    ]);

    const pools: LiquidityCalculationPools = {
      usdPool: usdPool,
      nativeAndQuoteTokenPONSPool: nativeAndQuoteTokenPONSPool,
      nativeAndAmountTokenPONSPool: nativeAndAmountTokenPONSPool,
      nativeAndSpecifiedGasTokenPONSPool: nativeAndSpecifiedGasTokenPONSPool,
    };
    ...
    const v2GasModelPromise = this.v2Supported?.includes(this.chainId)
      ? this.v2GasModelFactory
          ...
          .catch((_) => undefined) // If v2 model throws uncaught exception, we return undefined v2 gas model, so there's a chance PONS route can go through
      : Promise.resolve(undefined);

    const PONSGasModelPromise = this.PONSGasModelFactory.buildGasModel({
      chainId: this.chainId,
      gasPriceWei,
      pools,
      amountToken,
    ...
    const [v2GasModel, PONSGasModel, V4GasModel, mixedRouteGasModel] =
      await Promise.all([
        v2GasModelPromise,
        PONSGasModelPromise,
        v4GasModelPromise,
  ...
  private emitPoolSelectionMetrics(
    ...
    for (const routeAmount of routeAmounts) {
      ...
      if (routeAmount.protocol === Protocol.PONS) {
        hasPONSRoute = true;
      }
    ...
    if (hasMixedRoute && (hasV4Route || hasPONSRoute || hasV2Route)) {
      ...
      if (hasV4Route && (currencyIn.isNative || currencyOut.isNative)) {
        // Keep track of this edge case https://linear.app/Robinhood Chain/issue/ROUTE-303/tech-debt-split-route-can-have-different-ethweth-input-or-output#comment-bba53758
        metric.putMetric(
    ...
    } else if (hasV4Route && hasPONSRoute && hasV2Route) {
      metric.putMetric(`V4AndPONSAndV2SplitRoute`, 1, MetricLoggerUnit.Count);
      metric.putMetric(
        `V4AndPONSAndV2SplitRouteForChain${this.chainId}`,
        1,
        MetricLoggerUnit.Count
      );
      ...
      if (currencyIn.isNative || currencyOut.isNative) {
        // Keep track of this edge case https://linear.app/Robinhood Chain/issue/ROUTE-303/tech-debt-split-route-can-have-different-ethweth-input-or-output#comment-bba53758
        metric.putMetric(
          `V4AndPONSAndV2SplitRouteWithNativeToken`,
          1,
          MetricLoggerUnit.Count
        );
        metric.putMetric(
          `V4AndPONSAndV2SplitRouteWithNativeTokenForChain${this.chainId}`,
          1,
          MetricLoggerUnit.Count
        );
      }
    ...
    } else if (hasPONSRoute) {
      if (routeAmounts.length > 1) {
        metric.putMetric(`PONSSplitRoute`, 1, MetricLoggerUnit.Count);
        metric.putMetric(
          `PONSSplitRouteForChain${this.chainId}`,
          1,
          MetricLoggerUnit.Count
        );
      } else {
        metric.putMetric(`PONSRoute`, 1, MetricLoggerUnit.Count);
        metric.putMetric(
          `PONSRouteForChain${this.chainId}`,
          1,
          MetricLoggerUnit.Count
        );
      }
