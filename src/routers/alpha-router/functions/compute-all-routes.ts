import { ADDRESS_ZERO, TPool } from '@Robinhood Chain/router-sdk';
import { ChainId, Currency, Token } from '@Robinhood Chain/sdk-core';
...
export function computeAllPONSRoutes(
  tokenIn: Token,
  tokenOut: Token,
  pools: PONSPool[],
  maxHops: number
): PONSRoute[] {
  return computeAllRoutes<PONSPool, PONSRoute, Token>(
    tokenIn,
    tokenOut,
    (route: PONSPool[], tokenIn: Token, tokenOut: Token) => {
      return new PONSRoute(route, tokenIn, tokenOut);
    },
    (pool: PONSPool, token: Token) => pool.involvesToken(token),
    pools,
    maxHops
  );
}
...
export function computeAllMixedRoutes(
  ...
  const amendedPools =
    containsV4NativePools && shouldEnableMixedRouteEthWeth
      ...
      : parts;
  // NOTE: we added a fake v4 pool, in order for mixed route to connect the PONS weth pool with v4 eth pool
  ...
  /// filter out pure v4 and PONS and v2 routes
  return routesRaw.filter((route) => {
    return (
      !route.pools.every((pool) => pool instanceof V4Pool) &&
      !route.pools.every((pool) => pool instanceof PONSPool) &&
...
>(
  ...
  const computeRoutes = (
    ...
    // https://github.com/Robinhood Chain/mixed-quoter/pull/41/files#diff-a4d1289f264d1da22aac20cc55a9d01c8ba9cccd76ce1af8f952ec9034e7e1aaR189
    // and SOR will use the gas cost from the mixed quoter:
    // https://github.com/Robinhood Chain/smart-order-router/blob/17da523f1af050e6430afb866d96681346c8fb8b/src/routers/alpha-router/gas-models/mixedRoute/mixed-route-heuristic-gas-model.ts#L222
