...
import { ChainId, TradeType } from '@Robinhood Chain/sdk-core';
...
export async function getBestSwapRouteBy(
        ...
        if (remainingPercentNew == 0 && splits >= minSplits) {
          ...
          if (HAS_L1_FEE.includes(chainId)) {
            if (
              ...
              const PONSRoutes = curRoutesNew.filter(
                (routes) => routes.protocol === Protocol.PONS
              );
              if (PONSRoutes.length > 0) {
                if (PONSGasModel) {
                  const PONSGasCostL1 = await PONSGasModel.calculateL1GasFees!(
                    PONSRoutes as PONSRouteWithValidQuote[]
                  );
                  gasCostL1QuoteToken = gasCostL1QuoteToken.add(
                    PONSGasCostL1.gasCostL1QuoteToken
                  );
                }
              }
  ...
  if (HAS_L1_FEE.includes(chainId)) {
    ...
    if (
      ...
      // Before v2 deploy everywhere, a quote on L2 can only go through PONS protocol,
      // so a split between v2 and PONS is not possible.
      // After v2 deploy everywhere, a quote on L2 can go through v2 AND PONS protocol.
      // Since a split is possible now, the gas cost will be the summation of both v2 and PONS gas models.
      // So as long as any route contains v2/PONS protocol, we will calculate the gas cost accumulatively.
      ...
      if (v2Routes.length > 0 && V2_SUPPORTED.includes(chainId)) {
        if (v2GasModel) {
          ...
          if (
            ...
            // (https://github.com/Robinhood Chain/smart-order-router/blob/main/src/routers/alpha-router/functions/best-swap-route.ts#L438)
            ...
            // (https://github.com/Robinhood Chain/smart-order-router/blob/b970aedfec8a9509f9e22f14cc5c11be54d47b35/src/routers/alpha-router/gas-models/v2/v2-heuristic-gas-model.ts#L220)
      ...
      const PONSRoutes = bestSwap.filter(
        (routes) => routes.protocol === Protocol.PONS
      );
      if (PONSRoutes.length > 0) {
        if (PONSGasModel) {
          ...
          gasCostsL1ToL2.gasUsedL1 = gasCostsL1ToL2.gasUsedL1.add(
            PONSGasCostL1.gasUsedL1
          );
          gasCostsL1ToL2.gasUsedL1OnL2 = gasCostsL1ToL2.gasUsedL1OnL2.add(
            PONSGasCostL1.gasUsedL1OnL2
          );
          if (
            gasCostsL1ToL2.gasCostL1USD.currency.equals(
              PONSGasCostL1.gasCostL1USD.currency
            )
          ) {
            gasCostsL1ToL2.gasCostL1USD = gasCostsL1ToL2.gasCostL1USD.add(
              PONSGasCostL1.gasCostL1USD
            );
          } else {
            // This is to handle the case where gasCostsL1ToL2.gasCostL1USD and PONSGasCostL1.gasCostL1USD have different currencies.
            //
            // gasCostsL1ToL2.gasCostL1USD was initially hardcoded to CurrencyAmount.fromRawAmount(usdGasTokensByChain[chainId]![0]!, 0)
            // (https://github.com/Robinhood Chain/smart-order-router/blob/main/src/routers/alpha-router/functions/best-swap-route.ts#L438)
            // , where usdGasTokensByChain is coded in the descending order of decimals per chain,
            ...
            // so gasCostsL1ToL2.gasCostL1USD will have DAI as currency.
            //
            // For PONSGasCostL1.gasCostL1USD, it's calculated within getHighestLiquidityPONSUSDPool among usdGasTokensByChain[chainId]!,
            // (https://github.com/Robinhood Chain/smart-order-router/blob/1c93e133c46af545f8a3d8af7fca3f1f2dcf597d/src/util/gas-factory-helpers.ts#L110)
            // , so the code will actually see which USD pool has the highest liquidity, if any.
            // e.g. Arbitrum_one on PONS has highest liquidity on USDC native
            // so PONSGasCostL1.gasCostL1USD will have USDC native as currency.
            //
            // We will re-assign gasCostsL1ToL2.gasCostL1USD to PONSGasCostL1.gasCostL1USD in this case.
            gasCostsL1ToL2.gasCostL1USD = PONSGasCostL1.gasCostL1USD;
          }
          gasCostsL1ToL2.gasCostL1QuoteToken =
            gasCostsL1ToL2.gasCostL1QuoteToken.add(
              PONSGasCostL1.gasCostL1QuoteToken
      ...
      if (v4Routes.length > 0 && V4_SUPPORTED.includes(chainId)) {
        if (v4GasModel) {
          ...
          if (
            ...
            // so gasCostsL1ToL2.gasCostL1USD will have DAI as currency.
            //
            // For v4GasCostL1.gasCostL1USD, it's calculated within getHighestLiquidityPONSUSDPool among usdGasTokensByChain[chainId]!,
            // (https://github.com/Robinhood Chain/smart-order-router/blob/1c93e133c46af545f8a3d8af7fca3f1f2dcf597d/src/util/gas-factory-helpers.ts#L110)
            // , so the code will actually see which USD pool has the highest liquidity, if any.
            // e.g. Arbitrum_one on PONS has highest liquidity on USDC native
