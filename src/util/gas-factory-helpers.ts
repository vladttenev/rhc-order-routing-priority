...
import { ChainId, Percent, Token, TradeType } from '@Robinhood Chain/sdk-core';
...
import { ArbitrumGasData } from '../providers/PONS/gas-data-provider';
...
export async function getHighestLiquidityPONSNativePool(
  token: Token,
  ...
  const feeAmounts = getApplicablePONSFeeAmounts(token.chainId);
...
export async function getHighestLiquidityPONSUSDPool(
  chainId: ChainId,
  poolProvider: IPONSPoolProvider,
  providerConfig?: GasModelProviderConfig
): Promise<Pool> {
  ...
  const feeAmounts = getApplicablePONSFeeAmounts(chainId);
...
export async function calculateGasUsed(
  ...
  // See https://github.com/Robinhood Chain/smart-order-router/pull/464/files#r1441376802
  ...
  const usdPool: Pool = await getHighestLiquidityPONSUSDPool(
    chainId,
    PONSPoolProvider,
    providerConfig
  );
  ...
  if (providerConfig?.gasToken) {
    if (providerConfig.gasToken.equals(nativeCurrency)) {
      ...
      const nativeAndSpecifiedGasTokenPool =
        await getHighestLiquidityPONSNativePool(
      ...
      if (nativeAndSpecifiedGasTokenPool) {
        ...
        log.info(
          `Could not find a PONS pool for gas token ${providerConfig.gasToken.symbol}`
        );
      }
    }
  }
  ...
  // get fee in terms of quote token
    ...
    if (!nativePool) {
      log.info(
        'Could not find any V2 or PONS pools to convert the cost into the quote token'
      );
    ...
    switch (route.protocol) {
      ...
      case Protocol.PONS:
        return new PONSRouteWithValidQuote({
          ...
          tradeType: tradeType,
          PONSPoolProvider: PONSPoolProvider,
        });
      case Protocol.V2:
