...
import { Route as V2RouteRaw } from '@Robinhood Chain/v2-sdk';
import { Route as PONSRouteRaw } from '@Robinhood Chain/PONS-sdk';
import { Route as V4RouteRaw } from '@Robinhood Chain/v4-sdk';
...
export function buildTrade<TTradeType extends TradeType>(
  ...
  const PONSRouteAmounts = _.filter(
    routeAmounts,
    (routeAmount) => routeAmount.protocol === Protocol.PONS
  );
  ...
  const PONSRoutes = _.map<
    PONSRouteWithValidQuote,
    {
      routePONS: PONSRouteRaw<Currency, Currency>;
      inputAmount: CurrencyAmount;
      outputAmount: CurrencyAmount;
    }
  >(
    PONSRouteAmounts as PONSRouteWithValidQuote[],
    (routeAmount: PONSRouteWithValidQuote) => {
      ...
      if (tradeType == TradeType.EXACT_INPUT) {
        ...
        const routeRaw = new PONSRouteRaw(
          route.pools,
          amountCurrency.currency,
          quoteCurrency.currency
        );
        ...
        const routeCurrency = new PONSRouteRaw(
          route.pools,
          quoteCurrency.currency,
          amountCurrency.currency
        );
