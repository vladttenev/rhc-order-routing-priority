import { Protocol, TPool } from '@Robinhood Chain/router-sdk';
import { ChainId, Currency, Percent } from '@Robinhood Chain/sdk-core';
...
import { PONS_CORE_FACTORY_ADDRESSES } from './addresses';
...
export const poolToString = (pool: TPool): string => {
  if (pool instanceof V4Pool) {
    ...
    return ` -- ${pool.fee / 10000}% [${PONSPool.getAddress(
      pool.token0,
      pool.token1,
      pool.fee,
      undefined,
      PONS_CORE_FACTORY_ADDRESSES[pool.chainId]
    )}]`;
...
export const routeAmountsToString = (
  ...
  const routeStrings = _.map(routeAmounts, ({ protocol, route, amount }) => {
    ...
    /// @dev special case for MIXED routes we want to show user friendly V2+PONS instead
    return `[${
      protocol == Protocol.MIXED ? 'V2 + PONS + V4' : protocol
...
function poolIsInExcludedProtocols(
  ...
  if (pool instanceof V4Pool) {
    ...
    return excludedProtocolsFromMixed?.includes(Protocol.PONS) ?? false;
  } else if (pool instanceof Pair) {
