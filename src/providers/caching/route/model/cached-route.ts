...
import { Currency } from '@Robinhood Chain/sdk-core';
    ...
    switch (this.protocol) {
      case Protocol.V4:
        return (this.route as V4Route).pools
          ...
          .join('->');
      case Protocol.PONS:
        return (this.route as PONSRoute).pools
          .map(
            (pool) =>
              `[PONS]${pool.token0.address}/${pool.token1.address}/${pool.fee}`
          )
          .join('->');
      case Protocol.V2:
        ...
        return (this.route as MixedRoute).pools
          .map((pool) => {
            if (pool instanceof V4Pool) {
              ...
              return `[PONS]${pool.token0.address}/${pool.token1.address}/${pool.fee}`;
