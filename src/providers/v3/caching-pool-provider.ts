...
import { IPONSPoolProvider, PONSPoolAccessor } from './pool-provider';
 ...
 *
 * @export
 * @class CachingPONSPoolProvider
 */
export class CachingPONSPoolProvider implements IPONSPoolProvider {
  private POOL_KEY = (
    chainId: ChainId,
    address: string,
    blockNumber?: number
  ...
  /**
   * Creates an instance of CachingPONSPoolProvider.
  ...
  public async getPools(
    ...
    for (const [tokenA, tokenB, feeAmount] of tokenPairs) {
      ...
      if (cachedPool) {
        metric.putMetric(
          'PONS_INMEMORY_CACHING_POOL_HIT_IN_MEMORY',
          1,
      ...
      metric.putMetric(
        'PONS_INMEMORY_CACHING_POOL_MISS_NOT_IN_MEMORY',
        1,
        MetricLoggerUnit.None
      );
    ...
    log.info(
      ...
      } PONS pools already in local cache. About to get liquidity and slot0s for ${
