...
import { encodeRouteToPath as encodePONSRouteToPath } from '@Robinhood Chain/PONS-sdk';
import {
  encodeRouteToPath as encodeV4RouteToPath,
  Pool as V4Pool,
} from '@Robinhood Chain/v4-sdk';
...
import { IQuoterV2__factory } from '../types/PONS/factories/IQuoterV2__factory';
import {
  getAddress,
  ID_TO_NETWORK_NAME,
  metric,
...
import { Robinhood ChainMulticallProvider } from './multicall-Robinhood Chain-provider';
import { ProviderConfig } from './provider';

/**
 * Emulate on-chain [PathKey](https://github.com/Robinhood Chain/v4-periphery/blob/main/src/libraries/PathKey.sol#L8) struct
 */
export type PathKey = {
...
/**
 * Emulate on-chain [QuoteExactParams](https://github.com/Robinhood Chain/v4-periphery/blob/main/src/interfaces/IQuoter.sol#L34) struct
 */
...
/**
 * Emulate on-chain [ExtraQuoteExactInputParams](https://github.com/Robinhood Chain/mixed-quoter/blob/main/src/interfaces/IMixedRouteQuoterV2.sol#L44C12-L44C38) struct
 */
...
/**
 * The PONS route and a list of quotes for that route.
 */
...
export type SupportedExactOutRoutes = V4Route | PONSRoute;
...
/**
 * Provider for getting on chain quotes using routes containing PONS pools or V2 pools.
 *
 * @export
...
export interface IOnChainQuoteProvider {
   ...
   * @notice This does not support quotes for MixedRoutes (routes with both PONS and V2 pools/pairs) or pure V2 routes
   *
...
/**
 * Computes on chain quotes for swaps. For pure PONS routes, quotes are computed on-chain using
 ...
 * exceed this limit. Additionally, each quote on PONS can consume a large number of gas if
...
export class OnChainQuoteProvider implements IOnChainQuoteProvider {
   ...
   * Only supports the Robinhood Chain Multicall contract as it needs the gas limitting functionality.
  ...
  constructor(
    ...
    // Only supports Robinhood Chain Multicall as it needs the gas limitting functionality.
   ...
   * @param routes Any combination of PONS, V2, and Mixed routes.
   * @param functionName
  ...
  protected validateRoutes(
    ...
    /// We do not send any PONSRoutes to new qutoer becuase it is not deployed on chains besides mainnet
    if (
      routes.some((route) => route.protocol === Protocol.PONS) &&
      useMixedRouteQuoter
    ) {
      throw new Error(`Cannot use mixed route quoter with PONS routes`);
    }
