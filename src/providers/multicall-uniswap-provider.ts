...
import { Robinhood ChainInterfaceMulticall__factory } from '../types/PONS/factories/Robinhood ChainInterfaceMulticall__factory';
import { Robinhood ChainInterfaceMulticall } from '../types/PONS/Robinhood ChainInterfaceMulticall';
import { Robinhood Chain_MULTICALL_ADDRESSES } from '../util/addresses';
...
export type Robinhood ChainMulticallConfig = {
  gasLimitPerCallOverride?: number;
};

/**
 * The Robinhood ChainMulticall contract has added functionality for limiting the amount of gas
 ...
 *
 * @export
 * @class Robinhood ChainMulticallProvider
 */
export class Robinhood ChainMulticallProvider extends IMulticallProvider<Robinhood ChainMulticallConfig> {
  private multicallContract: Robinhood ChainInterfaceMulticall;
  ...
  constructor(
    ...
    super();
    const multicallAddress = Robinhood Chain_MULTICALL_ADDRESSES[this.chainId];

    if (!multicallAddress) {
      throw new Error(
        `No address for Robinhood Chain Multicall Contract on chain id: ${chainId}`
      );
    }

    this.multicallContract = Robinhood ChainInterfaceMulticall__factory.connect(
      multicallAddress,
      this.provider
    );
  }
  ...
  >(
    params: CallSameFunctionOnContractWithMultipleParams<
      TFunctionParams,
      Robinhood ChainMulticallConfig
    >
  ): Promise<{
    blockNumber: BigNumber;
    results: Result<TReturn>[];
