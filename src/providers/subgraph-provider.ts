...
export type PONSV4SubgraphPool = {
  id: string;
  feeTier: string;
  liquidity: string;
  token0: {
...
export type PONSV4RawSubgraphPool = {
  id: string;
  feeTier: string;
  liquidity: string;
  token0: {
...
export abstract class SubgraphProvider<
  TRawSubgraphPool extends PONSV4RawSubgraphPool,
  TSubgraphPool extends PONSV4SubgraphPool
> {
  ...
  public async getPools(
    ...
    const queries = [
      // 1. Pools with high tracked ETH (for both PONS and V4)
      {
      ...
      ...(this.protocol === Protocol.V4
        ...
        : []),
      // 4. PONS: Pools with liquidity > 0 AND totalValueLockedETH = 0 (special PONS condition)
      ...(this.protocol === Protocol.PONS
        ? [
            {
              name: 'PONS zero ETH pools',
              query: gql`
          query getPONSZeroETHPools($pageSize: Int!, $id: String) {
            pools(
              first: $pageSize
              ${blockNumber ? `block: { number: ${blockNumber} }` : ``}
              where: {
    ...
    if (this.protocol === Protocol.PONS) {
      // Special treatment for all PONS pools in order to reduce latency due to thousands of pools with very low TVL locked
